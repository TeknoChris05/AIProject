...
def load_emails(path):
    df = pd.read_csv(path).dropna(subset=["label", "subject", "body"])
    ...
    print(f"Loaded {len(df)} emails  ({n_phish} phishing, {n_legit} legitimate)")
    return df
...
def compute_metrics(results, strategy):
    valid = results[
        (results["predicted"] != "error") &
        (results["true_label"].notna()) &
        (results["true_label"].isin(["phishing", "legitimate"]))
    ]
